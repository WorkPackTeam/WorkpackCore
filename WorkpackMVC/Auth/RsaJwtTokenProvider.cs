...
namespace WorkpackMVC.Auth
{
  public class RsaJwtTokenProvider : ITokenProvider
  {
    ...
    public string CreateToken(User user, DateTime expiry)
    {
      ...
      ClaimsIdentity identity = new ClaimsIdentity(new GenericIdentity(user.UserCode, "jwt"));

      // TODO: Add whatever claims the user may have...

      identity.AddClaim(new Claim("UserId", Convert.ToString(user.UserID)));
      identity.AddClaim(new Claim("UserName", Convert.ToString(user.FirstName + " " + user.LastName)));
      identity.AddClaim(new Claim("UserCode", Convert.ToString(user.UserCode)));
      identity.AddClaim(new Claim("CompanyId", Convert.ToString(user.CompanyId)));
